#ifndef joewu_HAOKAN_REC_GRAPH_ENGINE_GRAPH_DEPENDENCY_H
#define joewu_HAOKAN_REC_GRAPH_ENGINE_GRAPH_DEPENDENCY_H
...
public:
    inline GraphDependency() noexcept = default;
    inline GraphDependency(GraphDependency&&) noexcept;

    // 【GraphProcessor::setup】【GraphProcessor::on_activate】阶段使用
    // 声明依赖可变性，在下一次声明前，会维持之前的设定，reset也不会清除
    // 存在可变依赖的情况下，依赖目标的GraphData不能存在其他依赖，即使是不可变依赖也不行
    // 只存在不可变依赖情况下，依赖数目没有上限
    // 打破唯一性约束的情况下，Graph的运行会返回失败
    inline void declare_mutable(bool is_mutable = true) noexcept;
    ...
    // 【GraphProcessor::setup】阶段使用
    // 声明为类型T的输入流，得到的输入流可以存在context中供运行时使用
    // 可变版本会相当于进行了可变声明，后续可以读取非常量的T
    template <typename T>
    inline InputChannel<T> declare_channel() noexcept;
    template <typename T>
    inline MutableInputChannel<T> declare_mutable_channel() noexcept;

    // 【GraphProcessor::setup】【GraphProcessor::on_activate】【GraphProcessor::process】阶段使用
    // 是否为可变依赖
    ...
    // 【GraphProcessor::setup】【GraphProcessor::on_activate】【GraphVertex::invoke】阶段使用
    // 是否为可变依赖
    ...
    // 直接获取底层对象，已作废
    // 后续应该使用独立的状态检测和取值进行
    inline __attribute__((deprecated)) const GraphData* target() const noexcept;
    inline __attribute__((deprecated)) GraphData* mutable_target() noexcept;
    inline __attribute__((deprecated)) const GraphData* condition() const noexcept;
    ///////////////////////////////////////////////////////////////////////////

private:
    inline GraphDependency(const GraphDependency&) noexcept;
    inline void source(GraphVertex& vertex) noexcept;
    inline void target(GraphData& data, bool is_mutable, bool is_essential) noexcept;
    ...
    //        1: 正常激活，进入完成状态
    //       <0: 激活失败
    inline int32_t activate(Stack<GraphData*>& activating_data) noexcept;
    inline void ready(GraphData* data, Stack<GraphVertex*>& runnable_vertexes) noexcept;
    // 检测依赖是否成立，实际读取原子变量
...
private:
    typedef typename ::joewu::feed::mlarch::babylon::ConcurrentTransientQueue<T>::ConstConsumer ConstConsumer;
    typedef typename ::joewu::feed::mlarch::babylon::ConcurrentTransientQueue<T>::ConstConsumeRange ConstConsumeRange;

public:
    // Queue::Consumer的轻量级包装，可以默认构造和移动
    ...
    inline ConstConsumeRange consume(uint32_t num) {
        return _consumer.consume(num);
    }

private:
    inline ChannelConsumer(ConstConsumer&& consumer, bool valid) noexcept :
        _consumer(::std::move(consumer)), _valid(valid) {}

    ConstConsumer _consumer;
    bool _valid;
...
private:
    typedef typename ::joewu::feed::mlarch::babylon::ConcurrentTransientQueue<T>::Consumer Consumer;
    typedef typename ::joewu::feed::mlarch::babylon::ConcurrentTransientQueue<T>::ConsumeRange ConsumeRange;
...
public:
    ...
    inline ConsumeRange consume(uint32_t num) {
        return _consumer.consume(num);
    }
...
public:
    ...
    inline const Queue& value() {
        auto* queue = _dependency->value<Queue>();
        if (queue == nullptr) {
...
private:
    inline InputChannel(GraphDependency& dependency) noexcept :
        ...
        _dependency->declare_type<Queue>();
    }

    static const ::std::unique_ptr<Queue> DEFAULT_CLOSED_EMPTY_QUEUE;
...
public:
    ...
    inline Queue& value() {
        auto* queue = _dependency->mutable_value<Queue>();
        if (queue == nullptr) {
            queue = DEFAULT_CLOSED_EMPTY_QUEUE.get();
        }
        return *queue;
...
template <typename T>
const ::std::unique_ptr<typename InputChannel<T>::Queue>
InputChannel<T>::DEFAULT_CLOSED_EMPTY_QUEUE = [] {
    auto* queue = new typename InputChannel<T>::Queue;
    queue->close();
    return ::std::unique_ptr<typename InputChannel<T>::Queue>(queue);
} ();

template <typename T>
const ::std::unique_ptr<typename MutableInputChannel<T>::Queue>
MutableInputChannel<T>::DEFAULT_CLOSED_EMPTY_QUEUE = [] {
    auto* queue = new typename MutableInputChannel<T>::Queue;
    queue->close();
    return ::std::unique_ptr<typename MutableInputChannel<T>::Queue>(queue);
} ();

} // graph
} // feed
} // joewu
#endif // joewu_HAOKAN_REC_GRAPH_ENGINE_GRAPH_DEPENDENCY_H

#include <joewu/graph/engine/dependency.hpp>
