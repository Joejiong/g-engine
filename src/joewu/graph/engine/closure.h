#ifndef joewu_HAOKAN_REC_GRAPH_ENGINE_GRAPH_CLOSURE_H 
#define joewu_HAOKAN_REC_GRAPH_ENGINE_GRAPH_CLOSURE_H  

#include <joewu/graph/engine/expect.h>

#include <joewu/feed/mlarch/babylon/function.h>
...
class Closure;
typedef ::std::function<void(Closure&&)> ClosureCallback;
...
public:
    ...
    inline void run(ClosureCallback* callback) noexcept;
...
private:
    ...
    inline int32_t invoke(ClosureCallback* callback) noexcept;
    // 标记finish，只有第一次标记成功
    // 标记成功时，如果已经注册了callback，会通过callback返回
    inline bool mark_finished(int32_t error_code, ClosureCallback*& callback) noexcept;
    void log_unfinished_data() const noexcept;

    static ClosureCallback* SEALED_CALLBACK;

    GraphExecutor* _executor;
    ::std::atomic<int64_t> _waiting_vertex_num {1};
    ::std::atomic<int64_t> _waiting_data_num {1};
    ::std::atomic<ClosureCallback*> _callback {nullptr};
    int32_t _error_code {0};

    ClosureCallback* _flush_callback {nullptr};
...
#endif //joewu_HAOKAN_REC_GRAPH_ENGINE_GRAPH_CLOSURE_H

#include <joewu/graph/engine/closure.hpp>
